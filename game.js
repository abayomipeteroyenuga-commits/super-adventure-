...
function resetWorld(){
 ...
 const bossNames=['MEGA BEAR','KING COBRA','GIANT GORILLA','SWAMP CROC','IRON RHINO','ALPHA WOLF','WAR BOAR','TIGER KING','ANCIENT BEAR','DESERT COBRA','RUIN APE','SWAMP TITAN','THUNDER RHINO','SHADOW WOLF','FORTRESS BOAR','MAGIC TIGER','DARK BEAR','STORM COBRA','GIANT KING','ULTIMATE TIGER'];
 ...
 if(finishCelebration>0||transitionLocked){
   ...
   if(finishCelebration<=0&&finishNextWorld){
     ...
     if(world<WORLD_COUNT-1){
       ...
       if(transitionId===worldTransitionId){playMusic();msg('🌍 WORLD '+(world+1)+'/'+WORLD_COUNT+' — '+themes[world].name)}
     }else{
       running=false;gameState='complete';stopAllMusic();
...
function drawBoss(b){
 ctx.save(); const t=performance.now()/220; const breath=1+Math.sin(t+b.x*.002)*.025; const step=b.vx!==0?Math.sin(t*1.4)*7:0;
 const dir=b.face||1; ctx.translate(b.x+55,b.y+58);ctx.scale(dir*1.42,1.42*breath);
 let body='#70452b',belly='#b87945',dark='#3b2418',light='#e7c29d';
 switch(b.type){case'bear':body='#5d3926';belly='#c18a57';dark='#2c1b13';light='#edc49b';break;case'cobra':body='#286a3d';belly='#b8df83';dark='#153d25';light='#dff0ad';break;case'gorilla':body='#37302e';belly='#77655d';dark='#1e1918';light='#b8a49a';break;case'crocodile':body='#326b3b';belly='#b5cf78';dark='#1d4326';light='#d8e79e';break;case'rhino':body='#656b70';belly='#b9bec2';dark='#373b3e';light='#e0e2e3';break;case'wolf':body='#4f5964';belly='#c9d0d7';dark='#28313a';light='#e7edf2';break;case'boar':body='#75412f';belly='#c98762';dark='#351c15';light='#f1c4a3';break;case'tiger':body='#d77b1e';belly='#ffd06b';dark='#542b0c';light='#ffe7a4';break;}
 // grounded shadow
 ctx.fillStyle='#0005';ctx.beginPath();ctx.ellipse(0,61,63,10,0,0,7);ctx.fill();
 // muscular full body
 ctx.fillStyle=b.hitFlash>0?'#fff':animalGradient(-35,45,light,body);ctx.beginPath();ctx.ellipse(0,5,50,38,0,0,7);ctx.fill();
 // legs with animated stride
 [[-37,step],[-14,-step],[13,step],[35,-step]].forEach(([x,s])=>{ctx.fillStyle=dark;ctx.beginPath();ctx.roundRect(x,24,15,35+s*.15,5);ctx.fill();ctx.fillStyle=body;ctx.beginPath();ctx.ellipse(x+7,57+s*.1,10,6,0,0,7);ctx.fill();});
 ctx.fillStyle=belly;ctx.beginPath();ctx.ellipse(4,10,28,27,0,0,7);ctx.fill();
 // head and species details
 if(b.type==='cobra'){
   ctx.fillStyle=body;ctx.beginPath();ctx.moveTo(-31,4);ctx.quadraticCurveTo(-53,-35,0,-70);ctx.quadraticCurveTo(53,-35,31,4);ctx.fill();
   ctx.strokeStyle=light;ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(-25,-22);ctx.quadraticCurveTo(0,-35,25,-22);ctx.stroke();
   ctx.fillStyle='#e54b45';ctx.beginPath();ctx.moveTo(0,5);ctx.lineTo(-4,27);ctx.lineTo(4,27);ctx.closePath();ctx.fill();
 }else if(b.type==='crocodile'){
   ctx.fillStyle=body;ctx.beginPath();ctx.ellipse(5,-25,39,25,0,0,7);ctx.fill();ctx.fillRect(-48,-15,55,15);
   ctx.fillStyle=belly;ctx.beginPath();ctx.ellipse(-22,-8,24,7,0,0,7);ctx.fill();
   ctx.fillStyle='#f5f0cf';for(let x=-42;x<2;x+=8){ctx.beginPath();ctx.moveTo(x,-3);ctx.lineTo(x+4,-3);ctx.lineTo(x+2,4);ctx.closePath();ctx.fill();}
 }else{
   ctx.fillStyle=animalGradient(-65,-5,light,body);ctx.beginPath();ctx.arc(5,-28,32,0,7);ctx.fill();
 }
 // ears / horns / mane
 if(['bear','gorilla','boar'].includes(b.type)){ctx.fillStyle=body;ctx.beginPath();ctx.arc(-23,-53,14,0,7);ctx.arc(23,-53,14,0,7);ctx.fill();}
 if(b.type==='wolf'){ctx.fillStyle=body;ctx.beginPath();ctx.moveTo(-28,-46);ctx.lineTo(-23,-78);ctx.lineTo(-6,-54);ctx.moveTo(28,-46);ctx.lineTo(23,-78);ctx.lineTo(6,-54);ctx.fill();}
 if(b.type==='tiger'){ctx.fillStyle=dark;ctx.beginPath();ctx.arc(5,-27,37,0,7);ctx.fill();ctx.fillStyle=body;ctx.beginPath();ctx.arc(5,-28,31,0,7);ctx.fill();}
 if(b.type==='rhino'){ctx.fillStyle=body;ctx.beginPath();ctx.moveTo(12,-48);ctx.lineTo(42,-73);ctx.lineTo(29,-45);ctx.fill();ctx.beginPath();ctx.moveTo(30,-38);ctx.lineTo(51,-47);ctx.lineTo(30,-25);ctx.fill();}
 // expressive eyes
 eye(-7,-30,8);eye(17,-30,8);
 // nose/mouth
 ctx.fillStyle=dark;ctx.beginPath();ctx.ellipse(9,-9,11,7,0,0,7);ctx.fill();ctx.fillStyle=light;ctx.beginPath();ctx.ellipse(18,-10,4,2.5,0,0,7);ctx.fill();ctx.strokeStyle=dark;ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(-5,4);ctx.quadraticCurveTo(9,14,21,4);ctx.stroke();
 // tiger stripes
 if(b.type==='tiger'){ctx.strokeStyle='#4a260e';ctx.lineWidth=5;for(let x=-28;x<=28;x+=14){ctx.beginPath();ctx.moveTo(x,-48);ctx.lineTo(x-7,-20);ctx.stroke();}}
 // boar tusks
 if(b.type==='boar'){ctx.fillStyle='#fff6df';ctx.beginPath();ctx.moveTo(-25,-7);ctx.quadraticCurveTo(-39,3,-28,11);ctx.moveTo(25,-7);ctx.quadraticCurveTo(39,3,28,11);ctx.fill();}
 // boss UI
 ctx.textAlign='center';ctx.font='900 22px Arial';ctx.fillText(b.emote,0,-92);
 ctx.fillStyle='#fff';ctx.fillRect(-62,-116,124,14);ctx.fillStyle='#d52222';ctx.fillRect(-62,-116,124*Math.max(0,b.hp/b.maxHp),14);ctx.strokeStyle='#111';ctx.lineWidth=3;ctx.strokeRect(-62,-116,124,14);
 ctx.font='900 13px Arial';ctx.fillStyle='#fff';ctx.strokeStyle='#111';ctx.lineWidth=3;ctx.strokeText(b.name,0,78);ctx.fillText(b.name,0,78);
 ctx.restore();
}
...
function start(){if(rafId){cancelAnimationFrame(rafId);rafId=0}stopAllMusic();for(const k in keys)keys[k]=false;lives=3;coins=0;stars=0;score=0;world=0;level=1;timeLeft=180;finishCelebration=0;finishNextWorld=false;finishEmote='🥳';transitionLocked=false;rescueSequence=0;rescueDone=false;shopOpen=false;paused=false;gameState='playing';document.getElementById('world').textContent='1/'+WORLD_COUNT;resetWorld();running=true;document.getElementById('start').style.display='none';last=performance.now();playMusic();rafId=requestAnimationFrame(loop)}
document.getElementById('play').onclick=()=>{initAudio();start()}; document.getElementById('audioBtn').onclick=()=>{initAudio();musicOn=!musicOn;document.getElementById('audioBtn').textContent=musicOn?'🔊':'🔇';if(musicOn)playMusic();else {stopAllMusic()}};document.getElementById('pauseBtn').onclick=()=>{if(!running)return;paused=!paused;msg(paused?'PAUSED':'RESUMED');if(paused){if(rafId){cancelAnimationFrame(rafId);rafId=0}draw();}else resumeLoop();};
